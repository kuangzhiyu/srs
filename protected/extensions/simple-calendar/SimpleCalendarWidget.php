...
class SimpleCalendarWidget extends CWidget {
	...
	public function dayIsReported($day) {
	
		
		
		$dateArr = array($this->year, $this->month, $day);
		$date = implode('-', $dateArr);
		$userId = Yii::app()->user->id;
		$isReported = false;
		$isReported = Report::model()->checkDayIsReported($date, $userId);
		$isReported = Report::model()->find(1);
		print_r($isReported);
	}
}
